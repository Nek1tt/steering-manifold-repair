...
1. **DPAR (Direction-Preserving Activation Repair).** Обычный Gaussian-denoiser при сильном steering частично улучшает связность просто потому, что отменяет сам steering. DPAR удаляет из поправки компоненту вдоль steering-вектора и сохраняет запрошенный `alpha` с численной точностью. В финальном плотном held-out sweep заранее включённый full-DPAR control (`beta=1`, не post-hoc выбор по held-out) дал локальный выигрыш на высоком concept: `F@C90 = 71.45` против `66.46` у additive steering (`+4.99`). Это локальный, а не универсальный Pareto-выигрыш.
